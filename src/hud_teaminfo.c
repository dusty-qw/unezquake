...
static cvar_t scr_teaminfo_show_mega	   = { "scr_teaminfo_show_mega",	"1" };
...
static int SCR_HudDrawTeamInfoPlayer(ti_player_t *ti_cl, float x, int y, int maxname, int maxloc, qbool width_only, float scale, const char* layout, int weapon_style, int show_ammo, int show_mega, int show_countdown, int armor_style, int powerup_style, int flag_style, int low_health, qbool proportional);
...
void SCR_HUD_DrawTeamInfo(hud_t *hud)
{
	...
	if (hud_teaminfo_weapon_style == NULL) {
		...
		hud_teaminfo_show_mega = HUD_FindVar(hud, "show_mega");
	...
	width = SCR_HudDrawTeamInfoPlayer(&ti_clients[0], 0, 0, maxname, maxloc, true, hud_teaminfo_scale->value, hud_teaminfo_layout->string, hud_teaminfo_weapon_style->integer, hud_teaminfo_show_ammo->integer, hud_teaminfo_show_mega->integer, hud_teaminfo_show_countdown->integer, hud_teaminfo_armor_style->integer, hud_teaminfo_powerup_style->integer, hud_teaminfo_flag_style->integer, hud_teaminfo_low_health->integer, hud_teaminfo_proportional->integer);
		...
		while (sorted_teams[k].name) {
			...
			for (j = 0; j < slots_num; j++) {
				...
				if (!strcmp(cl.players[i].team, sorted_teams[k].name)) {
					SCR_HudDrawTeamInfoPlayer(&ti_clients[i], x, _y, maxname, maxloc, false, hud_teaminfo_scale->value, hud_teaminfo_layout->string, hud_teaminfo_weapon_style->integer, hud_teaminfo_show_ammo->integer, hud_teaminfo_show_mega->integer, hud_teaminfo_show_countdown->integer, hud_teaminfo_armor_style->integer, hud_teaminfo_powerup_style->integer, hud_teaminfo_flag_style->integer, hud_teaminfo_low_health->integer, hud_teaminfo_proportional->integer);
					_y += FONTWIDTH * hud_teaminfo_scale->value;
				}
			}
			k++;
					...
					case 'H': // draw health, padding with space on right side
						if (!width_only) {
							if (isDeadCA){
								...
								snprintf(tmp, sizeof(tmp), "%s%d", (ti_cl->health < low_health ? "&cf00" : (show_mega && ti_cl->health > 100 ? "&c9cf" : txtclr)), ti_cl->health);
							}
...
void TeamInfo_HudInit(void)
{
	...
	Cvar_Register(&scr_teaminfo_show_mega);
